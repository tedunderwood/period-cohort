...
Now we actually do a grid search to find the best binwdiths for firstpub and birthyear. We select the model with highest overall r2, and in doing that use tenfold cross-validation *on unseen authors*.
...
for (varnum in seq(1, 200)){
  ...
  for (bywidth in seq(4, 24, 4)) {
    for (fpwidth in seq(4, 24, 4)) {
      ...
      modelstring <- paste(depvar, '~', bylabel, '+', fplabel)
  ...
  # agemse = at[1, 2] + at[2, 2] -- no, it's ageless
  
  for (rownum in seq(1, dim(at)[1])) {
...
write.csv(res.df, file = 'ageless_deltas3.csv', quote = FALSE, row.names = FALSE)
```
RESULTS:
